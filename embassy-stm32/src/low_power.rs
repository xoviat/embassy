...
use crate::interrupt;
use crate::rcc::{Config as RccConfig, REFCOUNT_STOP1, REFCOUNT_STOP2};
...
#[cfg(stm32wlex)]
static mut RCC_RESUME_CONFIG: Option<RccConfig> = None;
...
pub(crate) unsafe fn set_rcc_config(_config: RccConfig) {
    #[cfg(stm32wlex)]
    {
        RCC_RESUME_CONFIG = Some(_config);
    }
}
    ...
    pub(crate) unsafe fn on_wakeup_irq() {
        critical_section::with(|cs| {
            #[cfg(stm32wlex)]
            {
                ...
                use crate::rcc::init as rcc_init;
                ...
                if extscr.c1stop2f() || extscr.c1stopf() {
                    ...
                    RCC_RESUME_CONFIG.map(|config| rcc_init(config));
