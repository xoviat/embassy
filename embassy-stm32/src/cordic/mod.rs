...
/// q1.31 related
pub struct Cordic32<'d, 'a, T: Instance> {
    inner: &'a mut Cordic<'d, T>,
}
...
impl<'d, 'a, T: Instance> Cordic32<'d, 'a, T> {
    ...
    pub async fn async_calc<'b, W, R>(
        &mut self,
        mut write_dma: Peri<'b, W>,
        mut read_dma: Peri<'b, R>,
...
/// q1.15 related
pub struct Cordic16<'d, 'a, T: Instance> {
    inner: &'a mut Cordic<'d, T>,
}
