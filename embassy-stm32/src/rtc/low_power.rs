...
impl Rtc {
    pub(super) fn micros_from_unix_epoch(&self) -> i128 {
        RtcTimeProvider::new().now().unwrap().micros_from_unix_epoch()
    }

    pub(super) fn calc_epoch(&self) -> i128 {
        self.micros_from_unix_epoch() - Instant::now().as_micros() as i128
    }
    ...
    pub(crate) fn stop_wakeup_alarm(&mut self) -> embassy_time::Instant {
        ...
        Instant::from_micros((self.micros_from_unix_epoch() - self.epoch).try_into().unwrap())
    }
